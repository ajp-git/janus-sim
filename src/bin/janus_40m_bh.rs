/// Janus 40M Production Run — Pure Barnes-Hut (same as validated 8M)
///
/// 60M caused CUDA OOM on RTX 3060 12GB
/// 40M should fit with pure BH (no TreePM)
///
/// Parameters:
///   N = 40_000_000
///   box = 542.0 Mpc (same density as 20M/430Mpc)
///   θ = 0.7 (FIX-012 validated)
///   softening = 0.1 Mpc (default, validated on 8M)
///   dt = 0.01
...
const N_PARTICLES: usize = 40_000_000;
const BOX_SIZE: f64 = 542.0;  // Same density as 20M/430Mpc
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║   Janus 40M — Pure Barnes-Hut (same as 8M validated)           ║");
    println!("║   RTX 3060 12GB, no TreePM discontinuity                       ║");
    println!("╚════════════════════════════════════════════════════════════════╝\n");
    ...
    let output_dir = format!("/app/output/40M_bh_{}", date);
    ...
    println!("Creating BH simulation (40M particles)...");
    let t0 = Instant::now();
