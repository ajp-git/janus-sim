#!/usr/bin/env python3
"""
4K Composite Frame Renderer for Janus Simulation V3
"""
...
def render_composite(snap_path, output_path):
    ...
    pos, signs, step, z = read_snapshot(snap_path)
    P = compute_global_purity(pos, signs, BOX_SIZE)
    ...
    # Header - V3 with RANDOM signs
    fig.suptitle(f'V4 λ₀=2 Mpc  |  η={ETA}, λ₀={LAMBDA_0} Mpc  |  z={z:.2f}  |  P={P:.3f}  |  step {step}/{TOTAL_STEPS}',
    ...
    purity, density = compute_purity_map_2d(pos, signs, BOX_SIZE, n_cells=64)
...
def main():
    snap_dir = Path("/mnt/T2/janus-sim/output/champion_10m_v4/snapshots")
    out_dir = Path("/mnt/T2/janus-sim/output/champion_10m_v4/frames_composite")
    out_dir.mkdir(parents=True, exist_ok=True)

    processed = set()

    print("Starting V3 composite renderer...")
    ...
    while True:
        ...
        if any("02000" in s.name for s in snaps):
