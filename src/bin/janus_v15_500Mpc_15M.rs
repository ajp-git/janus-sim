//! JANUS V15 — Filament Persistence Test
//! 15M particles, L=500 Mpc, k_min=20, z=5→0
//! Goal: Test if higher resolution (15M vs 3M) allows filaments to persist to z=0
...
const N_GRID: usize = 246;  // 246³ = 14,886,936 ≈ 15M particles
const ETA: f64 = 1.045;
...
fn main() {
    ...
    let out = "/app/output/janus_v15_500Mpc_15M_kmin20";
    fs::create_dir_all(format!("{}/snapshots", out)).unwrap();
    fs::create_dir_all(format!("{}/fields", out)).unwrap();

    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║  JANUS V15 — FILAMENT PERSIST | 15M | k_min=20 | L=500  ║");
    println!("╚═══════════════════════════════════════════════════════════╝");
