//! Phase 10A.5a — Validation force-field cross-correlation r(k) :
//! TreePM Janus GPU vs PP-direct CPU sur Zel'dovich N=10K.
//!
//! Setup: 22³=10648 particules sur lattice + perturbation 15% dx, mélange
//! 5% m+ / 95% m- (Janus μ=19), L=100, n_pm=64, r_cut=9.375, r_s=1.875,
//! θ=0.5, softening=0.05, cosmologie neutre.
//!
//! Métrique: cross-correlation r(k) = ⟨F̂_GPU·F̂_PP⟩ / sqrt(⟨|F̂_GPU|²⟩·⟨|F̂_PP|²⟩)
//! par k mode. Critère: min r(k) > 0.99.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ========== PP-direct (CPU reference) ==========
    println!("Computing PP-direct (N²={:.2e} pairs)...", (n*n) as f64);
    let t0 = std::time::Instant::now();
    let mass_f64 = vec![1.0_f64; n];
    // PP-direct with sign-aware Janus interaction
    let signs_i32: Vec<i32> = signs.iter().map(|&s| s as i32).collect();
    let acc_pp = pp_direct_janus(&pos_f64, &mass_f64, &signs_i32, l, softening, 1.0,
        c_ratio_sq, phi, repulsion_scale);
    println!("  PP: {:.2}s", t0.elapsed().as_secs_f64());
    ...
    if min_r > 0.99 {
        println!("✅ Phase 10A.5a PASS — min r(k) > 0.99");
        Ok(())
    } else if min_r > 0.95 {
        println!("⚠ Phase 10A.5a GO_WITH_CAVEAT — min r(k) ∈ (0.95, 0.99]");
        Ok(())
    } else {
        eprintln!("❌ Phase 10A.5a FAIL — min r(k) = {:.4} < 0.95", min_r);
        std::process::exit(1)
    }
}
