...
# ── Sigma adaptatif basé sur l'espacement physique ───────────────────
def get_sigmas(N, box=BOX, canvas_px=S):
    """σ basé sur l'espacement moyen des particules en pixels.
    Pour 15M particules dans 500 Mpc: spacing ~ 2.5 Mpc ~ 11 px
    On veut σ_fine ~ 1-2 px, σ_glow ~ 6-10 px pour voir la structure.
    """
    spacing_mpc = box / max(N, 1)**(1.0/3.0)
    spacing_px  = spacing_mpc * canvas_px / box
    # Réduction drastique des σ pour grandes N
    sig_fine    = np.clip(spacing_px * 0.15, 1.0, 3.0)   # structure fine
    sig_glow    = np.clip(spacing_px * 0.8, 5.0, 12.0)   # nébuleuse modérée
    return sig_fine, sig_glow


# ── Splatting gaussien sur canvas carré ──────────────────────────────
def splat(px, py, pz, N_total, depth_sigma=120.0):
    """
    Projette sur canvas S×S. Coordonnées px,py en Mpc [-BOX/2, +BOX/2].
    Depth cuing Z centré sur 0.
    """
    # Shift coords from [-BOX/2, +BOX/2] to [0, BOX]
    px_shifted = px + BOX / 2.0
    py_shifted = py + BOX / 2.0

    # Pixel coords dans le canvas carré
    ix = (px_shifted / BOX * S).astype(np.float32)
    iy = (py_shifted / BOX * S).astype(np.float32)

    # Depth cuing centré sur z=0 (le centre de la boîte)
    dz      = np.abs(pz)  # pz déjà centré sur 0
    ...
    sig_fine, sig_glow = get_sigmas(N_total)
    fine = gaussian_filter(grid, sigma=sig_fine, truncate=4.0)
    glow = gaussian_filter(grid, sigma=sig_glow,  truncate=4.0)
    print(f"  σ_fine={sig_fine:.1f}px σ_glow={sig_glow:.1f}px  peak_raw={grid.max():.1f}", end='')
    return fine + glow * 0.35
...
def trails(px, py, vx, vy, speeds, trail_mpc=5.0, n_steps=12):
    ...
    for s in range(1, n_steps+1):
        t     = s / n_steps
        alpha = (1 - t)**1.5
        # Shift from [-BOX/2, +BOX/2] to [0, BOX]
        tx = ((px - vx_n*tlen*t + BOX/2) / BOX * S)
        ty = ((py - vy_n*tlen*t + BOX/2) / BOX * S)
...
def render(snap_path, step, z, with_trails=False):
    ...
    # Conversion Mpc → pixels frame (coords [-BOX/2, +BOX/2] → [0, S])
    def px_frame(mpc_x): return X_OFF + (mpc_x + BOX/2) / BOX * S
    def py_frame(mpc_y): return (mpc_y + BOX/2) / BOX * S   # origin lower dans l'ax
    ...
    outfile = f"{out_dir}/render_v4_step{step:04d}_{suffix}_4K.png"
