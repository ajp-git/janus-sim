#!/usr/bin/env python3
"""Render daemon for VSL Petit Production run"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
OUT_DIR = Path("/mnt/T2/janus-sim/output/vsl_petit_production/frames_10panel")
...
def render_frame(snap_path, out_path):
    ...
    fig.suptitle(f'Janus VSL Petit (μ=19, dynamic c²) | Step {step} | z={z:.3f} | c²={c_ratio_sq:.4f} | N={n//1e6:.1f}M',
    ...
    fig.text(0.5, 0.01, f"ΔZ={dz:.0f} Mpc | Peak ρ={peak_density:.0f} | c²={c_ratio_sq:.4f}",
...
def main():
    print(f"""
╔═══════════════════════════════════════════════════╗
║  Render Daemon - VSL Petit Production             ║
╚═══════════════════════════════════════════════════╝
Output: {OUT_DIR}
""")
    rendered = set()
