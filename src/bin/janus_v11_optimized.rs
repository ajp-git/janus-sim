//! JANUS V11 OPTIMIZED — A/B Validation with Performance Optimizations
//!
//! This run tests numerical optimizations for performance improvement.
//!
//! Optimizations applied:
//!   1. Adaptive θ: 0.5 (step<500), 0.7 (500≤step<1500), 0.9 (step≥1500)
//!   2. Reduced R_cut: 15 Mpc (was 18 Mpc)
//!
//! Expected: ~2x speedup with <5% physics deviation
...
fn get_adaptive_theta(step: usize) -> f64 {
    if step < 500 {
        0.5  // Conservative early on
    } else if step < 1500 {
        0.7  // Standard
    } else {
        0.9  // Aggressive late (structures formed)
    }
}
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  JANUS V11 OPTIMIZED — A/B Performance Test                  ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!("  R_cut = {} Mpc (OPTIMIZED: was 18)", R_CUT);
    println!("  dt = {}", DT);
    println!("  z_init = {}", Z_INIT);
    println!("  steps = {}", TOTAL_STEPS);
    println!();
    println!("Optimizations:");
    println!("  1. Adaptive θ: 0.5→0.7→0.9 based on step");
    println!("  2. Reduced R_cut: 15 Mpc (was 18)");
    println!();

    // Output directory
    let output_dir = "/app/output/janus_v11_optimized";
    ...
    println!("Generating Zel'dovich ICs (seed=42)...");
    let t0 = Instant::now();
    ...
    // Set initial theta (will be updated adaptively)
  ...
  "z_init": {},
  "total_steps": {},
  "optimizations": ["adaptive_theta", "reduced_r_cut"]
}}"#, N_GRID, n_particles, L_BOX, ETA, ALPHA_IC, EPSILON,
     R_CUT, DT, Z_INIT, TOTAL_STEPS);
    ...
    println!("\n--- Running {} steps with OPTIMIZATIONS ---\n", TOTAL_STEPS);
    ...
    for step in 1..=TOTAL_STEPS {
        ...
        // TreePM step with Hubble friction (using optimized R_cut)
    ...
    println!();
    println!("═══════════════════════════════════════════════════════════════");
    println!("  OPTIMIZED SIMULATION COMPLETE");
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    println!("Results:");
    ...
    println!("  Reference V11: ~300 ms/step");
    println!("  Speedup: {:.1}x", 300.0 / (total_time * 1000.0 / TOTAL_STEPS as f64));
    println!();
  ...
  "optimizations": ["adaptive_theta", "reduced_r_cut_15"]
}}"#,
        total_time / 60.0,
    ...
    println!();
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  Compare with janus_v11_reference for A/B validation         ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
}
...
fn main() {
    println!("CUDA + cuFFT required. Use: cargo run --release --features \"cuda cufft\" --bin janus_v11_optimized");
}
