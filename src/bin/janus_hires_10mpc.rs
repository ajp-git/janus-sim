//! High-resolution 5M Janus simulation - μ = 19, Box = 10 Mpc
//!
//! Usage: cargo run --release --features "cuda cufft" --bin janus_hires_10mpc
...
const N_TOTAL: usize = 5_000_000;  // 5M particles
const MU: f64 = 19.0;
const BOX_SIZE: f64 = 10.0;   // 10 Mpc
const Z_INIT: f64 = 4.0;
const DT: f64 = 0.005;
const STEPS: usize = 4000;  // Full production run (~2h)
const THETA: f64 = 0.9;    // Validated theta
const SOFTENING: f64 = 0.001;  // Validated softening
const SEED: u64 = 42;
const SNAPSHOT_INTERVAL: usize = 40;  // Every 40 steps (101 total)
const CSV_INTERVAL: usize = 50;
const R_CUT: f64 = 2.0;  // Smaller r_cut for smaller box
...
#[cfg(feature = "cuda")]
fn main() {
    let run_name = "hires_10mpc".to_string();

    println!("================================================================");
    println!("  JANUS 5M HIGH-RES 10Mpc — μ = {}", MU);
    println!("================================================================");
    ...
    for step in 0..=STEPS {
        ...
        if step > 0 {
            ...
            if step <= 10 || step % 100 == 0 {
                println!("  step {} TreePM: {:.0}ms", step, t0.elapsed().as_millis());
            }
        }
  ...
  "n_snapshots": {}
    ...
    println!("\n================================================================");
    println!("  μ={} 10Mpc PRODUCTION COMPLETE", MU);
    println!("================================================================");
