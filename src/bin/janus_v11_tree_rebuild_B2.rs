//! JANUS V11 — Tree Rebuild A/B Test (B2)
//!
//! Tests reduced Barnes-Hut tree rebuild frequency optimization
//! Rebuilds tree every 3 steps instead of every step
//!
//! Parameters: IDENTICAL to V11 reference
//!   - 500K particles (80³ grid)
//!   - L = 200 Mpc
//!   - z_init = 5 → z = 0
//!   - TreePM with proper H(τ) from CosmoInterpolator
//!
//! Optimization under test:
//!   - Tree rebuild interval = 3 (instead of 1)
//!   - On steps 0, 3, 6, ... rebuild trees
//!   - On steps 1, 2, 4, 5, ... reuse cached trees
...
// V11 PARAMETERS (IDENTICAL TO REFERENCE)
...
const THETA: f64 = 0.7;
const R_CUT: f64 = 18.0;           // TreePM split scale (SAME as reference)
const DT: f64 = 0.01;
...
const TREE_REBUILD_INTERVAL: usize = 3;  // Rebuild every 3 steps

#[cfg(all(feature = "cuda", feature = "cufft"))]
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  JANUS V11 — TREE REBUILD A/B TEST (B2)                       ║");
    println!("║  Optimization: Rebuild tree every {} steps                     ║", TREE_REBUILD_INTERVAL);
    println!("╚══════════════════════════════════════════════════════════════╝");
    println!();

    let n_particles = N_GRID * N_GRID * N_GRID;
    println!("Parameters (identical to V11 reference):");
    println!("  N_GRID = {} → N = {} ({:.1}K)", N_GRID, n_particles, n_particles as f64 / 1e3);
    println!("  L = {} Mpc", L_BOX);
    println!("  η = {}", ETA);
    println!("  α_IC = {}", ALPHA_IC);
    println!("  ε = {} Mpc", EPSILON);
    println!("  θ = {} (constant)", THETA);
    ...
    println!("  Tree rebuild interval = {} steps", TREE_REBUILD_INTERVAL);
    println!("  Expected speedup: ~{}x on BH portion", TREE_REBUILD_INTERVAL);
    println!();

    // Output directory
    let output_dir = "/app/output/janus_v11_tree_rebuild_B2";
    ...
    // Generate Zel'dovich ICs (SAME seed as reference)
    println!("Generating Zel'dovich ICs (seed=42, same as reference)...");
    let t0 = Instant::now();
  ...
  "version": "V11_tree_rebuild_B2",
  "optimization": "tree_rebuild_interval=3",
  "N_GRID": {},
  "N": {},
  "L_box_mpc": {},
  "eta": {},
  ...
  "tau_range": {}
     ...
     THETA, R_CUT, DT, Z_INIT, TOTAL_STEPS, TREE_REBUILD_INTERVAL,
    ...
    writeln!(ts_file, "step,tau,z,a,H,KE,KE_ratio,segregation,rebuild,dt").unwrap();

    // Save initial snapshot
    save_snapshot(&sim, &format!("{}/snapshots/snap_000000.bin", output_dir), n_particles);
    println!("  → Saved snapshot: snap_000000.bin");

    // Run simulation with tree caching
    println!("\n--- Running {} steps with tree rebuild every {} steps ---\n", TOTAL_STEPS, TREE_REBUILD_INTERVAL);
    println!("{:>6} {:>8} {:>8} {:>10} {:>10} {:>10} {:>7} {:>8}",
             "Step", "z", "a", "H", "KE/KE₀", "Seg", "Rebuild", "Time");
    println!("{}", "─".repeat(80));

    let run_start = Instant::now();
    let mut seg_max = seg0;
    let mut seg_max_step = 0;
    let mut total_rebuild_time_ms: u128 = 0;
    let mut total_reuse_time_ms: u128 = 0;
    ...
    for step in 1..=TOTAL_STEPS {
        ...
        // Decide whether to rebuild tree this step
        // Rebuild on steps 1, 4, 7, 10, ... (every 3 steps starting from 1)
        // This ensures first step builds fresh trees
        let rebuild_tree = (step - 1) % TREE_REBUILD_INTERVAL == 0;

        // TreePM step with tree caching
        if let Err(e) = sim.step_treepm_gpu_cached(DT, R_CUT, h, dtau_per_dt, rebuild_tree) {
        ...
        // Track timing by mode
        if rebuild_tree {
            total_rebuild_time_ms += step_ms;
            rebuild_count += 1;
        } else {
            total_reuse_time_ms += step_ms;
            reuse_count += 1;
        }
        ...
        if step % LOG_INTERVAL == 0 {
            ...
            let rebuild_str = if rebuild_tree { "YES" } else { "no" };

            // Write to time series
            writeln!(ts_file, "{},{:.6},{:.4},{:.6},{:.6},{:.6e},{:.6},{:.6},{},{:.4}",
                     step, current_tau, z, a, h, ke, ke_ratio, seg, rebuild_str, DT).unwrap();

            // Print to console
            println!("{:>6} {:>8.3} {:>8.5} {:>10.6} {:>10.4} {:>10.4} {:>7} {:>6}ms",
                     step, z, a, h, ke_ratio, seg, rebuild_str, step_ms);
        }
    ...
    let avg_rebuild_ms = if rebuild_count > 0 { total_rebuild_time_ms / rebuild_count } else { 0 };
    let avg_reuse_ms = if reuse_count > 0 { total_reuse_time_ms / reuse_count } else { 0 };

    println!();
    println!("═══════════════════════════════════════════════════════════════");
    println!("  TREE REBUILD A/B TEST (B2) COMPLETE");
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    println!("Results:");
    ...
    println!("  Rebuild steps: {} (avg {}ms/step)", rebuild_count, avg_rebuild_ms);
    println!("  Reuse steps: {} (avg {}ms/step)", reuse_count, avg_reuse_ms);
    if avg_rebuild_ms > 0 && avg_reuse_ms > 0 {
        let speedup = avg_rebuild_ms as f64 / avg_reuse_ms as f64;
        println!("  Reuse speedup: {:.2}x", speedup);
    }
    println!();
...
fn main() {
    println!("CUDA + cuFFT required. Use: cargo run --release --features \"cuda cufft\" --bin janus_v11_tree_rebuild_B2");
}
