//! VSL Bimetric Phase 1 — Scale-up Run
//!
//! 2M particles, 10000 steps, snapshot at EVERY step.
...
// Phase 1 Parameters
const N_PLUS: usize = 1_000_000;
const N_MINUS: usize = 1_000_000;
...
#[cfg(feature = "cuda")]
fn run_phase1() {
    let output_dir = "/app/output/vsl_bimetric_phase1";
    create_dir_all(format!("{}/snapshots", output_dir)).unwrap();

    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║  PHASE 1 lancée (2M) — pipeline automatique P0→P1→P2 actif      ║");
    ...
    let csv_path = format!("{}/evolution_phase1.csv", output_dir);
    ...
    writeln!(log_writer, "VSL Bimetric Phase 1 — μ={} η={}", MU, ETA).unwrap();
    ...
    println!("Initializing GPU simulation (2M particles)...");
    ...
    println!("\n╔══════════════════════════════════════════════════════════════════╗");
    if all_criteria_met {
        println!("║  PHASE 1 COMPLETE — GO                                          ║");
        println!("╠══════════════════════════════════════════════════════════════════╣");
        println!("║  → Prêt pour Phase 2 (10M particules)                            ║");
    } else {
        println!("║  PHASE 1 COMPLETE — NO-GO                                        ║");
        println!("╠══════════════════════════════════════════════════════════════════╣");
    ...
    let result_path = format!("{}/phase1_result.txt", output_dir);
...
// Helper functions (same as Phase 0)
#[cfg(feature = "cuda")]
