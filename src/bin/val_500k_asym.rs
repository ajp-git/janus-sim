//! Validation 500K — Asymmetric ICs: δ- = 2 × δ+
//!
//! Test avec amplitude de déplacement asymétrique entre espèces.
//! - Particules + : amplitude δ normale
//! - Particules - : amplitude 2 × δ
//!
//! Usage:
//!   cargo run --release --features cuda,cufft --bin val_500k_asym
...
// Asymmetric amplitude factor
const MINUS_AMP_FACTOR: f64 = 2.0;  // δ- = 2 × δ+
...
#[cfg(all(feature = "cuda", feature = "cufft"))]
fn main() {
    let run_name = "val_500k_asym2x";
    let output_dir = format!("/app/output/{}", run_name);
    fs::create_dir_all(&output_dir).expect("Failed to create output dir");

    println!("═══════════════════════════════════════════════════════════════");
    println!("  VALIDATION 500K — ASYMMETRIC ICs (δ- = 2×δ+)");
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    ...
    println!("  H      = {:.2}", HUBBLE);
    println!("  η      = {:.3}", ETA);
    println!("  δ-/δ+  = {:.1}", MINUS_AMP_FACTOR);
    println!("  steps  = {}", TOTAL_STEPS);
    println!("  output = {}", output_dir);
    println!();

    // Generate ICs with asymmetric amplitudes
    println!("Generating asymmetric Zel'dovich ICs...");
    let seed = 42u64;
    let (positions, velocities, signs) = generate_zeldovich_ics_asymmetric(seed, ETA, K_MIN);
...
#[cfg(all(feature = "cuda", feature = "cufft"))]
fn save_snapshot(sim: &GpuNBodyTwoPass, step: usize, output_dir: &str, _n_positive: usize, n_total: usize) {
...
// ═══════════════════════════════════════════════════════════════════════════
// ZEL'DOVICH IC GENERATION — ASYMMETRIC δ- = 2×δ+
// ═══════════════════════════════════════════════════════════════════════════

fn generate_zeldovich_ics_asymmetric(seed: u64, eta: f64, k_min: usize) -> (Vec<f64>, Vec<f64>, Vec<i32>) {
    let mut rng = StdRng::seed_from_u64(seed);
    let ng = N_GRID;
    let ng3 = ng * ng * ng;
    let cell = L_BOX / ng as f64;

    // FIRST: Assign signs randomly with ratio eta = N-/N+
    ...
    for ix in 0..ng {
        for iy in 0..ng {
            for iz in 0..ng {
                ...
                // ASYMMETRIC AMPLITUDE: δ- = 2 × δ+
                let amp_factor = if signs[idx] > 0 { 1.0 } else { MINUS_AMP_FACTOR };

                // Displacement with species-dependent amplitude
