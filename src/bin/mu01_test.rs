//! Quick test: μ=0.1 (91% m+, 9% m-)
//! Pure gravity, 2M particles, 500 steps
...
const MU: f64 = 0.1;  // N-/N+ = 0.1 → N+ = 91%, N- = 9%
...
fn main() {
    println!("================================================================");
    println!("  TEST μ=0.1 — m+ dominated (91% / 9%)");
    println!("================================================================");
    println!("  N = {} ({:.1}M)", N, N as f64 / 1e6);
    println!("  μ = {} → N+ ≈ 91%, N- ≈ 9%", MU);
    println!("  Box = {} Mpc, δ_init = {}%", BOX_SIZE, DELTA_INIT * 100.0);
    ...
    println!("Expected: N+ = {}, N- = {}", n_plus, n_minus);
    ...
    println!("  Actual: N+ = {} ({:.1}%), N- = {} ({:.1}%)",
             actual_n_plus, 100.0 * actual_n_plus as f64 / N as f64,
             actual_n_minus, 100.0 * actual_n_minus as f64 / N as f64);

    // Output
    let output_dir = "/app/output/mu01_test";
    ...
    writeln!(csv_file, "step,t_gyr,rho_plus_max_ratio,corr_delta,v_plus_kms").unwrap();
    ...
    for step in 0..=STEPS {
        ...
        if step % CSV_INTERVAL == 0 {
            ...
            let (rho_max, corr, v_plus) = compute_metrics(&pos, &vel, &sgn, BOX_SIZE, N_CELLS);

            writeln!(csv_file, "{},{:.4},{:.2},{:.4},{:.1}",
                     step, t_gyr, rho_max, corr, v_plus).unwrap();
            ...
            println!("  step {:3} | t={:.2} Gyr | ρ+max/ρ̄={:.2} ({:+.1}%) | v+={:.0} km/s {}",
                     step, t_gyr, rho_max, growth, v_plus, alert);
...
fn compute_metrics(positions: &[f32], velocities: &[f32], signs: &[i8],
                   box_size: f64, n_cells: usize) -> (f64, f64, f64) {
