//! VSL Production Run - Petit's True Model
//! Dynamic c_ratio: c_ratio_sq(t) = a⁺(t)/a⁻(t)
//! μ = 19 (canonical Petit value)
...
fn main() {
    ...
    let output_dir = "/app/output/vsl_petit_production";
    ...
    // Dynamic c_ratio from Petit MPLA 2014
    let c_ratio_sq_init = CoupledFriedmann::c_ratio_sq_at_z(Z_INIT, ETA);
    let c_ratio_init = c_ratio_sq_init.sqrt();

    // Thermal velocities - different for m+ and m-
    let sigma_v_plus = (K_B_OVER_MP_CODE * T_INIT_PLUS / MU_MOL).sqrt();
    // For m-, scale by c_ratio (slower thermal motion in negative sector)
    let sigma_v_minus = sigma_v_plus * c_ratio_init;

    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║       VSL PRODUCTION — PETIT'S TRUE MODEL                            ║");
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    println!("║  μ = {} (canonical Petit value)", MU);
    println!("║  c_ratio = DYNAMIC: c_ratio_sq(z) = (1+z)^δ, δ = {:.4}", (ETA-1.0)/ETA);
    ...
    println!("║  η = {}, Ω_b = {}, δ_init = {}%", ETA, OMEGA_B, DELTA_INIT * 100.0);
    ...
    println!("║    N_stars > 0   → ★★★ FIRST JANUS STAR");
    println!("║    v_rms- > 5000 → ⚠ check stability");
    println!("╚══════════════════════════════════════════════════════════════════════╝\n");
    ...
    println!("GPU init: {:.2}s, initial c_ratio = {:.6}",
             start_time.elapsed().as_secs_f64(), c_ratio_init);
    ...
    writeln!(csv, "step,t_Gyr,z,rho_plus_max,rho_minus_max,v_rms_plus,v_rms_minus,n_stars,segregation,c_ratio_sq").unwrap();

    println!("  Step |  t_Gyr |      z | ρ+_max | ρ-_max |   v_rms+ |   v_rms- | N_stars |    Seg | c_ratio² | Status");
    println!("-------|--------|--------|--------|--------|----------|----------|---------|--------|----------|--------");
    ...
    for step in 0..=N_STEPS {
        ...
        if do_metric || do_snapshot {
            ...
            if v_rms_minus > 5000.0 {
                status = format!("{} ⚠ v_rms-!", status);
            }
            ...
            if do_metric {
                println!("{:>6} | {:>6.3} | {:>6.3} | {:>6} | {:>6} | {:>8.0} | {:>8.0} | {:>7} | {:>6.4} | {:>8.4} | {} ({:.1}h)",
                         step, t_gyr, z, rho_plus_max, rho_minus_max,
                         v_rms_plus, v_rms_minus, 0, seg, c_ratio_sq, status, eta_h);

                writeln!(csv, "{},{:.4},{:.4},{},{},{:.0},{:.0},{},{:.6},{:.6}",
                         step, t_gyr, z, rho_plus_max, rho_minus_max,
                         v_rms_plus, v_rms_minus, 0, seg, c_ratio_sq).unwrap();
                csv.flush().unwrap();
            }
