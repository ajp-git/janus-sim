//! JANUS FINAL PRODUCTION RUN — 10M PARTICLES
//!
//! Validated parameters from test_1m_zeldovich (12 April 2026)
//! Dual-seed Zel'dovich ICs for m+ (42) and m- (43)
...
// ============================================================================
// PRODUCTION PARAMETERS — 10M FINAL RUN
// ============================================================================

const N_PARTICLES: usize = 10_000_000;
const L_BOX: f64 = 300.0;           // Mpc (resolution ~0.3 Mpc)
const ETA: f64 = 1.045;             // Validated Janus mass ratio
const Z_INIT: f64 = 5.0;            // Validated in 1M test (z=10 exceeds Janus limit)
const DT: f64 = 0.005;              // Gyr
const STEPS: usize = 1500;          // z=5 → z=0 (scaled from 1M test)
const THETA: f64 = 0.7;             // Tree opening angle (precision)
const SOFTENING: f64 = 0.15;        // Softening [Mpc] (scaled for 300 Mpc)
const R_CUT: f64 = 30.0;            // TreePM cutoff [Mpc]
const PM_GRID: usize = 256;         // PM grid resolution
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║           JANUS FINAL PRODUCTION RUN — 10M PARTICLES                 ║");
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    println!("║  N = 10,000,000  |  L = 300 Mpc  |  η = 1.045                        ║");
    println!("║  z: 5 → 0        |  θ = 0.7      |  ε = 0.15 Mpc                     ║");
    println!("║  Seeds: m+ = 42, m- = 43  |  n_s = 0.965  |  δ_rms = 0.1             ║");
    println!("║  TreePM: PM 256³ + BH cutoff 30 Mpc                                  ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝");
    println!();

    // Output directory
    let base_dir = std::path::Path::new("/app/output/run_final_10m");
    ...
    println!("Estimated runtime: 40-100 hours GPU");
    println!("══════════════════════════════════════════════════════════════════════════");
    println!("{:>6} {:>8} {:>6} {:>8} {:>8} {:>10} {:>8} {:>8} {:>10}",
             "step", "z", "S", "corr", "v_ratio", "ρ_max+", "P", "status", "ETA");
    println!("──────────────────────────────────────────────────────────────────────────");
    ...
    for step in 0..=STEPS {
        ...
        if step % CSV_INTERVAL == 0 || step % SNAPSHOT_INTERVAL == 0 {
            ...
            if step % 10 == 0 {
                ...
                println!("{:>6} {:>8.3} {:>6.4} {:>8.4} {:>8.4} {:>10.2e} {:>8.3} {:>8} {:>10}",
                    step, z, seg, corr, vrms_ratio, rho_max_plus, purity, status, eta_str);
            }
        ...
        if step > 0 && step % 100 == 0 {
            ...
            let rate = step as f64 / elapsed;
            let remaining = (STEPS - step) as f64 / rate;
            println!("  ═══ Progress: {}/{} steps ({:.1}%), {:.1}h elapsed, ~{:.1}h remaining ═══",
                step, STEPS, 100.0 * step as f64 / STEPS as f64,
                elapsed / 3600.0, remaining / 3600.0);
        }
    }
    ...
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    println!("║  Steps: {:>6}  |  z_final: {:>6.4}  |  Time: {:>6.1} hours            ║",
        STEPS, final_z, elapsed.as_secs_f64() / 3600.0);
    println!("╚══════════════════════════════════════════════════════════════════════╝");
    ...
    "n_s": {},
    "delta_rms": {},
    "validated_by": "test_1m_zeldovich (12 April 2026)"
}}"#,
        N_PARTICLES, L_BOX, ETA, Z_INIT, final_z, STEPS,
        elapsed.as_secs_f64() / 3600.0, seg, corr, vrms_plus_kms, vrms_minus_kms,
    ...
    println!();
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║  🎉 JANUS 10M PRODUCTION RUN COMPLETE — READY FOR ANALYSIS           ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝");
}
