//! Petit Pure 20M — 1000 Mpc box — μ=8, λ=0
//!
//! Same as petit_pure_20m but with 1000 Mpc box to avoid
//! octree/box resonance artifact at L/8 = 62.5 Mpc (k=8 spike).
//!
//! With 1000 Mpc: L/8 = 125 Mpc → resonance at different physical scale
//! Density is 8× lower → Jeans trigger slightly later but clean spectrum
...
const BOX_SIZE: f64 = 1000.0;  // 1000 Mpc (vs 500 in original)
...
fn main() {
    println!("========================================================");
    println!("  PETIT PURE 20M — 1000 Mpc — μ={}, λ=0", MU);
    println!("========================================================");
    println!("  Avoiding octree resonance (k=8 spike @ 500 Mpc)");
    println!("========================================================");
    ...
    println!("  Box = {} Mpc (2× larger), z = {} -> 0", BOX_SIZE, Z_INIT);
    ...
    // Setup output
    let base_dir = std::path::Path::new("/app/output/petit_pure_20m_1000mpc");
    ...
    for step in 0..=STEPS {
        ...
        if step % SNAPSHOT_INTERVAL == 0 {
            ...
            println!("  step {:4} | z={:.3} | P={:.3} | void={:.1}% | wall={:.1}% | blobs={} | r={:.1}Mpc",
                     step, z, purity, void_frac * 100.0, wall_frac * 100.0, n_blobs, r_blob_mean);
    ...
    println!();
    println!("========================================================");
    println!("  PETIT PURE 20M — 1000 Mpc — COMPLETE");
    println!("========================================================");
