#!/usr/bin/env python3
"""
10-Panel Render Daemon for VSL Bimetric Phase 0
Watches snapshots and renders 10-panel frames
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
SNAP_DIR = Path("/mnt/T2/janus-sim/output/vsl_bimetric_phase0/snapshots")
OUT_DIR = Path("/mnt/T2/janus-sim/output/vsl_bimetric_phase0/frames_10panel")
OUT_DIR.mkdir(exist_ok=True, parents=True)

ZOOM_SIZE = 30.0  # Zoom window size in Mpc (smaller for 100 Mpc box)
GRID_SIZE = 64
CONTOUR_BINS = 64
DENSITY_VMIN = 1
DENSITY_VMAX = 200
...
def render_projection(ax, x, y, half, color, title, s=0.02, alpha=0.3, subsample=100000):
    n = len(x)
    if n > subsample:
...
def render_zoom_with_contours(ax, x_p, y_p, x_m, y_m, x0, y0, zoom_half, title):
    ...
    if len(xz_p) > 50000:
        idx = np.random.choice(len(xz_p), 50000, replace=False)
...
def render_zoom(ax, x, y, x0, y0, zoom_half, color, title, s=0.15, alpha=0.6):
    ...
    xz, yz = x[mask], y[mask]
    if len(xz) > 50000:
        idx = np.random.choice(len(xz), 50000, replace=False)
        xz, yz = xz[idx], yz[idx]
...
def render_frame(snap_path, out_path):
    try:
        ...
        fig.suptitle(f'Janus VSL Option B | Step {step} | z={z:.3f} | N={n/1e6:.1f}M | Box={box:.0f}Mpc',
        ...
        ax.set_xlabel('X [Mpc]', color='gray', fontsize=8)
        ax.set_ylabel('Z [Mpc]', color='gray', fontsize=8)

        ax = fig.add_subplot(gs[0, 2], facecolor='#0a0a14')
        render_zoom_with_contours(ax, snap['x'][mp], snap['y'][mp], snap['x'][mm], snap['y'][mm], x0, y0, zoom_half, 'XY ZOOM m+ + m- contours')
        ax.set_xlabel('X [Mpc]', color='gray', fontsize=8)
        ax.set_ylabel('Y [Mpc]', color='gray', fontsize=8)
...
def main():
    print("10-Panel Render Daemon for Phase 0 started")
    print(f"Watching: {SNAP_DIR}")
    print(f"Output: {OUT_DIR}")

    rendered = set()

    # First pass: render existing files
    while True:
        result_file = SNAP_DIR.parent / "phase0_result.txt"
        sim_complete = result_file.exists()
        ...
        if sim_complete:
            ...
            print(f"\nRender complete: {n_frames} frames in {OUT_DIR}")
            break
