//! JANUS — Segregation vs Box Size Test
//!
//! Tests whether segregation plateau is invariant to box size:
//!   - L = 100 Mpc
//!   - L = 200 Mpc (reference)
//!   - L = 400 Mpc
//!
//! Usage: cargo run --release --features "cuda cufft" --bin janus_seg_boxsize 200
...
const ETA: f64 = 1.06;             // Fixed η (valid cosmology)
...
#[cfg(all(feature = "cuda", feature = "cufft"))]
fn main() {
    // Get box size from command line
    let args: Vec<String> = env::args().collect();
    let l_box: f64 = if args.len() > 1 {
        args[1].parse().unwrap_or(200.0)
    } else {
        println!("Usage: {} <L_box>", args[0]);
        println!("  Example: {} 200", args[0]);
        println!("  Recommended values: 100, 200, 400 (Mpc)");
        return;
    };

    // Validate box size
    if l_box < 50.0 || l_box > 1000.0 {
        println!("ERROR: L_box must be in range [50, 1000] Mpc, got {}", l_box);
        return;
    }

    // Scale softening and R_cut with box size
    let epsilon = 0.18 * l_box / 200.0;  // Scale with box
    let r_cut = 18.0 * l_box / 200.0;    // Scale with box

    let run_name = format!("L{}", l_box as i32);

    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  JANUS — SEGREGATION vs BOX SIZE TEST                        ║");
    println!("║  L = {} Mpc                                                  ║", l_box);
    println!("╚══════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!("  L = {} Mpc ← TEST VARIABLE", l_box);
    println!("  η = {}", ETA);
    println!("  α_IC = {} (fixed)", ALPHA_IC);
    println!("  ε = {:.3} Mpc (scaled with L)", epsilon);
    println!("  θ = {}", THETA);
    println!("  R_cut = {:.1} Mpc (scaled with L)", r_cut);
    println!("  dt = {}", DT);
    ...
    // Output directory
    let output_dir = format!("/app/output/seg_boxsize_{}", run_name);
    ...
    println!("Generating Zel'dovich ICs (seed=42, L={})...", l_box);
    let t0 = Instant::now();
    let (pos, vel, signs) = generate_zeldovich_ics(42, N_GRID, l_box);
  ...
  "N_negative": {},
  "KE_initial": {}
}}"#, l_box, ETA, n_particles, ALPHA_IC, epsilon,
  ...
  "N_negative": {}
    ...
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  L = {} Mpc  →  Seg_1000 = {:.4}                            ║", l_box, seg_final);
    println!("╚══════════════════════════════════════════════════════════════╝");
}

#[cfg(all(feature = "cuda", feature = "cufft"))]
fn generate_zeldovich_ics(seed: u64, n_grid: usize, l_box: f64) -> (Vec<f64>, Vec<f64>, Vec<i32>) {
...
fn main() {
    println!("CUDA + cuFFT required. Use: cargo run --release --features \"cuda cufft\" --bin janus_seg_boxsize <L_box>");
}
