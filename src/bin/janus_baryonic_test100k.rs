//! JANUS BARYONIC TEST 100K - Physics Validation Test
//!
...
// ═══════════════════════════════════════════════════════════════════════════
const N_PARTICLES: usize = 100_000;  // TEST: 100K particles
const L_BOX: f64 = 100.0;       // [Mpc] - smaller box for faster test
const Z_INIT: f64 = 4.0;        // Initial redshift
const DT: f64 = 0.001;          // Time step [Gyr]
const N_STEPS: usize = 200;     // TEST: 200 steps only
const THETA: f64 = 0.7;         // Barnes-Hut opening angle
const EPSILON: f64 = 0.05;      // Softening [Mpc] - smaller for smaller box
...
// OUTPUT
// ═══════════════════════════════════════════════════════════════════════════
const SNAPSHOT_INTERVAL: usize = 50;  // TEST: every 50 steps
const METRIC_INTERVAL: usize = 1;     // TEST: every step for detailed monitoring
const OUTPUT_DIR: &str = "/app/output/janus_baryonic_test100k";
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════════════════╗");
    println!("║        JANUS BARYONIC TEST 100K — PHYSICS VALIDATION                     ║");
    println!("╠══════════════════════════════════════════════════════════════════════════╣");
    println!("║  COSMOLOGY:");
    println!("║    μ = {}, η = {}", MU, ETA);
    ...
    for step in 0..=N_STEPS {
        ...
        if do_metric || do_snapshot {
            ...
            if do_metric {
                ...
                if step % (METRIC_INTERVAL * 10) == 0 || step <= 50 {
                ...
                if ratio > 1.15 {
                    println!("\n  ⚠ AUTO-STOP: v_rms ratio = {:.4} > 1.15", ratio);
                    println!("    m- runaway detected at step {}, z={:.2}", step, z);
                    break;
                }
            }
