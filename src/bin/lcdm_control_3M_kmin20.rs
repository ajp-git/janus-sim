//! LCDM CONTROL — Numerical control run (all masses positive)
//! 3M particles, L=500 Mpc, k_min=20, z=5→0
//! Goal: Show r(k) ≈ 0 (no anti-correlation) vs Janus r(k) ≈ -0.5
...
const N_GRID: usize = 144;  // 144³ = 2,985,984 ≈ 3M particles
const ETA: f64 = 1.045;
const ALPHA_IC: f64 = 1.6;  // standard velocity amplitude
const EPSILON: f64 = 0.25;  // softening for 3M
const THETA: f64 = 0.5;     // more accurate tree
const R_CUT: f64 = 40.0;
const DT: f64 = 0.01;
const Z_INIT: f64 = 5.0;
const STEPS: usize = 5000;
const SNAP_INT: usize = 5;
const LOG_INT: usize = 10;
const K_MIN_IC: usize = 20; // ICs: start at mode 20 → λ_min = 25 Mpc (filament scale)

#[cfg(all(feature = "cuda", feature = "cufft"))]
fn main() {
    let n = N_GRID * N_GRID * N_GRID;
    let out = "/app/output/lcdm_control_500Mpc_3M_kmin20";
    fs::create_dir_all(format!("{}/snapshots", out)).unwrap();
    fs::create_dir_all(format!("{}/fields", out)).unwrap();

    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║  LCDM CONTROL — ALL POSITIVE MASSES | 3M | k_min=20     ║");
    println!("╚═══════════════════════════════════════════════════════════╝");
    println!("N={} ({}³) η={} ε={} θ={} R_cut={} dt={}",
             n, N_GRID, ETA, EPSILON, THETA, R_CUT, DT);
    println!("ICs: k_min={} → λ_min = {:.0} Mpc (filament scale)", K_MIN_IC, L_BOX / K_MIN_IC as f64);
    ...
    sim.set_pm_k_min(2);  // PM force: full gravitational forces
    ...
    writeln!(ts, "0,{:.4},{:.5},{:.5},1.0,{:.4},{:.4},{:.2},{:.2}",
             1.0/a0-1.0, a0, h0, seg0, sp, lj, xi).unwrap();
    ...
    println!("ICs k_min={} enabled filament-scale perturbations", K_MIN_IC);
    println!("Output: {}", out);
}
    ...
    for iz in 0..ng {
        for iy in 0..ng {
            for ix in 0..ng {
                ...
                // ICs: seed modes from K_MIN_IC to k_max (filament scales only)
                // k_max ≈ 40 (Nyquist: km < 0.5 → m < 40 for L=500 Mpc)
                const K_MAX_IC: usize = 40;
                let n_modes = (K_MAX_IC - K_MIN_IC + 1) as f64;
                let amp = ALPHA_IC * 0.015 / n_modes.sqrt();  // constant amplitude

                for m in K_MIN_IC..=K_MAX_IC {
                    let km = m as f64 * k;
                    if km < 0.5 {
                        dx += amp * (km * x0 + px).sin();
                        dy += amp * (km * y0 + py).sin();
                        dz += amp * (km * z0 + pz).sin();
                        vx += amp * km * (km * x0 + px).cos();
                        vy += amp * km * (km * y0 + py).cos();
                        vz += amp * km * (km * z0 + pz).cos();
                    }
                }
                ...
                // LCDM: 99.9% positive, 0.1% negative (GPU needs both populations)
                signs.push(if rng.random::<f64>() < 0.001 { -1 } else { 1 });
            }
        }
    }
    (pos, vel, signs)
