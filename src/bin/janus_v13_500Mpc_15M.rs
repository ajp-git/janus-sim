//! JANUS V13 — High Resolution Test
//! 15M particles, L=500 Mpc, z=5→0
//! Goal: Test if σ_P=0.15 from 5M run is artifact or real physics
...
const N_GRID: usize = 247;  // 247³ ≈ 15,069,223 particles
const ETA: f64 = 1.045;
const ALPHA_IC: f64 = 1.6;
const EPSILON: f64 = 0.35;  // scaled for 15M: smaller softening
const THETA: f64 = 0.7;
const R_CUT: f64 = 40.0;    // scaled
const DT: f64 = 0.01;
...
fn main() {
    ...
    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║  JANUS V13 — HIGH RES | 15M | L=500 | z=5→0              ║");
    println!("╚═══════════════════════════════════════════════════════════╝");
    println!("N={} ({}³) η={} ε={} θ={} R_cut={} dt={}",
             n, N_GRID, ETA, EPSILON, THETA, R_CUT, DT);
    println!("Goal: Test if σ_P=0.15 (from 5M) is resolution artifact");
    ...
    println!("\nCOMPARISON (σ_P at z=0):");
    println!("  5M run:  σ_P = 0.27 (grid 64³)");
    println!("  15M run: σ_P = {:.2} (grid 64³)", sp_f);
    println!("  If σ_P increased significantly → 5M was under-resolved");
    println!("Output: {}", out);
}
