//! Filaments 18M — Configuration optimisée pour structures multi-échelles
//!
//! Paramètres :
//!   - 18M particules (262³)
//!   - k_min = 4×k_box (supprime modes k=1,2,3)
//!   - P(k) = k^-2 × exp(-(k/k_cut)²) avec k_cut = 0.25 Mpc⁻¹
//!   - ε = 0.15 Mpc (softening fin)
//!   - Attribution des signes ALÉATOIRE
//!   - Objectif: seg₀ ≤ 1e-4, virial_ratio ~ 10-20
...
// ═══════════════════════════════════════════════════════════════════════════
// PHYSICAL PARAMETERS — FILAMENTS 18M CONFIGURATION
// ═══════════════════════════════════════════════════════════════════════════

const N_GRID: usize = 262;              // 262³ ≈ 17.96M particles
const L_BOX: f64 = 492.0;               // Mpc
const Z_INIT: f64 = 5.0;
const SOFTENING: f64 = 0.15;            // Reduced for filament resolution

// k_box = 2π/L = 0.01277 Mpc⁻¹
const K_BOX: f64 = 2.0 * PI / L_BOX;

// P(k) truncation — suppress k=1,2,3 modes (k_min = 4×k_box)
const K_MIN: f64 = 4.0 * K_BOX;         // = 0.0511 Mpc⁻¹
const K_CUT: f64 = 0.25;                // Mpc⁻¹ (exponential cutoff)
...
const ETA: f64 = 1.045;
const N_SAMPLE_VIRIALIZE: usize = 100000;

// Power spectrum amplitude (adjust for target virial_ratio ~ 10-20)
const AMPLITUDE: f64 = 0.008;

/// Generate Zel'dovich ICs with:
///   - P(k) = k^-2 × exp(-(k/k_cut)²)
///   - k_min = 4×k_box (no k=1,2,3)
...
fn generate_filament_ics(seed: u64) -> (Vec<f64>, Vec<f64>, Vec<i32>, usize) {
    ...
    println!("═══════════════════════════════════════════════════════════════");
    println!("  FILAMENTS 18M — Enhanced Cosmic Web Configuration");
    println!("═══════════════════════════════════════════════════════════════");
    println!("  Grid: {}³ = {} particles ({:.2}M)", N_GRID, n3, n3 as f64 / 1e6);
    println!("  Box: {} Mpc", L_BOX);
    println!("  z_init = {}", Z_INIT);
    println!();
    println!("  P(k) = k^-2 × exp(-(k/k_cut)²)");
    println!("  k_box = 2π/L = {:.5} Mpc⁻¹", K_BOX);
    println!("  k_min = 4×k_box = {:.5} Mpc⁻¹ (λ_max = {:.0} Mpc)", K_MIN, 2.0 * PI / K_MIN);
    println!("  k_cut = {:.2} Mpc⁻¹ (λ_cut = {:.0} Mpc)", K_CUT, 2.0 * PI / K_CUT);
    println!("  Softening ε = {} Mpc", SOFTENING);
    println!("  RANDOM sign assignment (target seg₀ ≤ 1e-4)");
    println!();
    ...
    for iz in 0..N_GRID {
        for iy in 0..N_GRID {
            for ix in 0..N_GRID {
                ...
                // Suppression of k < k_min (modes k=1,2,3)
                if k < K_MIN {
    ...
    println!("  Modes suppressed (k<k_min): {} (k=1,2,3)", n_modes_suppressed);
...
fn main() {
    println!("\n══════════════════════════════════════════════════════════════════════════");
    println!("  FILAMENTS 18M — Enhanced Cosmic Web Janus Simulation");
    println!("══════════════════════════════════════════════════════════════════════════\n");

    let seed = 12345u64;  // Different seed from 12M run
    let output_dir = "/app/output/filaments_18m";
    ...
    println!("  θ = {}", THETA);
    println!("  ε = {} Mpc", SOFTENING);

    // Check initial segregation BEFORE virialization
    let seg_pre_virial = sim.segregation_distance().expect("segregation failed");
    println!("\n  Seg BEFORE virial: {:.6} (target ≤ 1e-4)", seg_pre_virial);
    if seg_pre_virial > 1e-4 {
        println!("  ⚠️ WARNING: seg₀ > 1e-4, random assignment may have bias");
    } else {
        println!("  ✓ seg₀ ≤ 1e-4 OK");
    }
    ...
    println!("═══════════════════════════════════════════════════════════════");
    println!("  N = {} ({:.2}M)", n3, n3 as f64 / 1e6);
    println!("  Box = {} Mpc", L_BOX);
    println!("  Steps = {}", TOTAL_STEPS);
    println!("  θ = {}, ε = {} Mpc", THETA, SOFTENING);
    println!("  k_box = {:.5} Mpc⁻¹", K_BOX);
    println!("  k_min = 4×k_box = {:.5} Mpc⁻¹ (suppresses k=1,2,3)", K_MIN);
    println!("  k_cut = {:.2} Mpc⁻¹", K_CUT);
    println!("  P(k) = k^-2 × exp(-(k/{:.2})²)", K_CUT);
    println!("  Sign assignment: RANDOM");
    println!("═══════════════════════════════════════════════════════════════");
    ...
    println!("\nInitial state:");
    println!("  KE₀ = {:.4e}", ke_0);
    println!("  Seg₀ = {:.6} (should be ≤1e-4 with random signs)", seg_0);
    ...
    for step in 1..=TOTAL_STEPS {
        ...
        if step <= 10 || step % 100 == 0 {
            ...
            println!("Step {:5}: z={:.2}, KE/KE₀={:.4}, Seg={:.6} ({:.0} ms, ETA {:.1}h)",
                     step, z, ke_ratio, seg, step_ms, eta_h);
        }

        // Milestones
        if step == 500 || step == 1000 || step == 2000 || step == 5000 || step == 10000 {
            println!("\n>>> MILESTONE step {} <<<", step);
            println!("  z = {:.2}", z);
            println!("  KE/KE₀ = {:.4}", ke_ratio);
            println!("  Seg = {:.6} (max = {:.6} @ z={:.2})", seg, seg_max, z_at_seg_max);
            csv.flush().unwrap();
        }
    ...
    println!("\n══════════════════════════════════════════════════════════════════════════");
    println!("  RUN COMPLETE — FILAMENTS 18M");
    println!("══════════════════════════════════════════════════════════════════════════");
    println!("  Runtime: {:.1}h", elapsed.as_secs_f64() / 3600.0);
    println!("  KE/KE₀ final: {:.4}", final_ke / ke_0);
    println!("  Seg₀ = {:.6}", seg_0);
    println!("  Seg_max = {:.6} @ z={:.2}", seg_max, z_at_seg_max);
    println!("  Seg final = {:.6}", final_seg);

    // Summary JSON
    let summary = format!(r#"{{
  "model": "Filaments 18M — Enhanced Cosmic Web Janus",
  "n_particles": {},
  "n_positive": {},
  "n_negative": {},
  "eta": {},
  ...
  "runtime_hours": {:.2}
        ...
        n3, n_positive, n_negative, ETA, L_BOX, K_BOX, K_MIN, K_CUT, SOFTENING, TOTAL_STEPS,
...
fn main() {
    eprintln!("Requires cuda feature: cargo run --release --features cuda --bin filaments_18m");
}
