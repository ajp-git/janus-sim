/// Janus 60M Production Run — Pure Barnes-Hut (same as validated 8M)
///
/// TreePM introduces force discontinuity at r_cut → KE explosion
/// Solution: Pure BH like validated 8M run (N_max = 63M on RTX 3060)
///
/// Parameters:
///   N = 60_000_000
///   box = 843.0 Mpc (same density as 8M: spacing 2.15 Mpc)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║   Janus 60M — Pure Barnes-Hut (same as 8M validated)           ║");
    println!("║   N_max=63M RTX 3060 (FIX-013), no TreePM discontinuity        ║");
    println!("╚════════════════════════════════════════════════════════════════╝\n");
    ...
    let output_dir = format!("/app/output/60M_bh_{}", date);
    ...
    println!("Creating BH simulation (60M particles)...");
    let t0 = Instant::now();
