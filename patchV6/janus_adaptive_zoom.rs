...
fn generate_zeldovich_ics(n_grid: usize, l_box: f64, z_init: f64, h0: f64, mu: f64) -> (Vec<f64>, Vec<f64>, Vec<i32>) {
    ...
    // avec conditions périodiques. ψ est stocké sur grille n_grid³ avec indexation
    // idx = iz * n_grid * n_grid + iy * n_grid + ix, coordonnées centrées [-L/2, L/2].
    let cic_interp = |field: &[f64], x: f64, y: f64, z: f64| -> f64 {
        // Coordonnées en unités de cellule, repère [0, n_grid)
        let fx = ((x + half_box) / spacing) % (n_grid as f64);
        let fy = ((y + half_box) / spacing) % (n_grid as f64);
        let fz = ((z + half_box) / spacing) % (n_grid as f64);
        let fx = if fx < 0.0 { fx + n_grid as f64 } else { fx };
        let fy = if fy < 0.0 { fy + n_grid as f64 } else { fy };
        let fz = if fz < 0.0 { fz + n_grid as f64 } else { fz };

        let ix0 = fx.floor() as usize % n_grid;
        let iy0 = fy.floor() as usize % n_grid;
        let iz0 = fz.floor() as usize % n_grid;
        let ix1 = (ix0 + 1) % n_grid;
        let iy1 = (iy0 + 1) % n_grid;
        let iz1 = (iz0 + 1) % n_grid;

        let dx = fx - fx.floor();
        let dy = fy - fy.floor();
        let dz = fz - fz.floor();

        let idx = |i: usize, j: usize, k: usize| k * n_grid * n_grid + j * n_grid + i;
