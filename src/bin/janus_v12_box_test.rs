//! JANUS V12 — Box Size Scaling Test
//!
//! Goal: Determine if characteristic scale ξ is intrinsic or box artifact
//!
//! Parameters:
//!   - 1M particles (100³ grid)
//!   - L = 400 Mpc (doubled from V11's 200 Mpc)
//!   - z_init = 5 → z = 0
//!   - TreePM with proper H(τ) from CosmoInterpolator
//!
//! Expected outcomes:
//!   Case A (intrinsic): ξ ≈ 30 Mpc (same as V11)
//!   Case B (artifact):  ξ ≈ 60 Mpc (scales with L)
...
// V12 PARAMETERS - doubled box size
const L_BOX: f64 = 400.0;          // Mpc (was 200 in V11)
const N_GRID: usize = 100;         // 100³ = 1,000,000 particles
const ETA: f64 = 1.06;
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  JANUS V12 — BOX SIZE SCALING TEST                           ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
    println!();

    let n_particles = N_GRID * N_GRID * N_GRID;
    println!("Parameters:");
    println!("  N_GRID = {} → N = {} ({:.2}M)", N_GRID, n_particles, n_particles as f64 / 1e6);
    println!("  L = {} Mpc (2× V11)", L_BOX);
    println!("  η = {}", ETA);
    ...
    // Output directory
    let output_dir = "/app/output/janus_v12_box_test";
  ...
  "dtau_per_dt": {},
  "tau_range": {},
  "comparison": "V11 used L=200 Mpc, V12 uses L=400 Mpc"
  ...
  "segregation_max_step": {}
    ...
    // Analysis guidance
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║  ANALYSIS: Compare ξ with V11                                ║");
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  V11: L = 200 Mpc → ξ ≈ 31 Mpc                               ║");
    println!("║  V12: L = 400 Mpc → ξ = ?                                    ║");
    println!("║                                                              ║");
    println!("║  If ξ ≈ 30 Mpc → intrinsic Janus scale                       ║");
    println!("║  If ξ ≈ 60 Mpc → box size artifact                           ║");
    println!("╚══════════════════════════════════════════════════════════════╝");
}
...
fn main() {
    println!("CUDA + cuFFT required. Use: cargo run --release --features \"cuda cufft\" --bin janus_v12_box_test");
}
