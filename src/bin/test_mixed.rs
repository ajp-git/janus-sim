/// Test mixed precision GPU simulation
/// cargo run --release --features cuda --bin test_mixed -- 30000000

#[cfg(feature = "cuda")]
fn main() {
    use janus::nbody_gpu_mixed::GpuNBodyMixed;
    ...
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║   Mixed Precision GPU Test (f32 pos/vel, f64 acc/tree)         ║");
    println!("╚════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    let t0 = Instant::now();
    let mut sim = match GpuNBodyMixed::new(n_positive, n_negative, box_size) {
        Ok(s) => s,
        Err(e) => {
    ...
    println!("  S = {:.6}", seg);
    println!();
    println!("✓ Mixed precision test completed successfully!");
}
