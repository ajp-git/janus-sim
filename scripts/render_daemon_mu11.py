#!/usr/bin/env python3
"""Render daemon for μ=11 production run"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
SNAP_DIR = Path("/mnt/T2/janus-sim/output/vsl_mu11_10M_production/snapshots")
OUT_DIR = Path("/mnt/T2/janus-sim/output/vsl_mu11_10M_production/frames_10panel")
...
def render_frame(snap_path, out_path):
    ...
    fig.suptitle(f'Janus VSL (μ=11) | Step {step} | z={z:.3f} | N={n//1e6:.1f}M | Peak@({x0:.0f},{y0:.0f},{z0:.0f})',
    ...
    fig.text(0.5, 0.01, f"ΔZ={dz:.0f} Mpc | Peak ρ={peak_density:.0f}",
...
def main():
    print(f"""
╔═══════════════════════════════════════════════════╗
║  Render Daemon - VSL μ=11 Production              ║
╚═══════════════════════════════════════════════════╝
Output: {OUT_DIR}
""")
    rendered = set()
