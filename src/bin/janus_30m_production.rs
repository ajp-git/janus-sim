//! Production 30M Janus simulation with frequent snapshots
//!
//! μ = 64, Box = 1000 Mpc, N = 30M
//! Snapshots every 5 steps for movie generation
//!
//! Usage: cargo run --release --features "cuda cufft" --bin janus_30m_production
...
// Production parameters - 20M max for 12GB VRAM
...
const SEED: u64 = 42;
const SNAPSHOT_INTERVAL: usize = 5;    // Every 5 steps for movie (401 snapshots × 500MB = 200GB)
const CSV_INTERVAL: usize = 50;  // Reduce logging for long run
...
fn main() {
    println!("================================================================");
    println!("  JANUS 30M PRODUCTION — μ = {}", MU);
    println!("================================================================");
    ...
    // Resolution check
    let d_mean_plus = (BOX_SIZE.powi(3) / n_positive as f64).powf(1.0/3.0);
    let d_mean_minus = (BOX_SIZE.powi(3) / n_negative as f64).powf(1.0/3.0);
    println!("\n  d_mean(m+) = {:.2} Mpc", d_mean_plus);
    println!("  d_mean(m-) = {:.2} Mpc", d_mean_minus);
    ...
    println!("\nInitializing GPU simulation for 30M particles...");
    ...
    println!("\n  t_start = {:.4} Gyr (z={})", t_start, Z_INIT);
    println!("  t_end = {:.4} Gyr (z=0)", t_end);
    println!("  dt = {:.4} Gyr", dt_gyr);

    let start = Instant::now();
    println!("\n================================================================");
    println!("  Starting 30M evolution...");
    println!("================================================================\n");
  ...
  "n_snapshots": {}
        ...
        0.0, diff_pois, corr_delta, exc_var_minus, t_inst,
    ...
    println!("\n================================================================");
    println!("  JANUS 30M PRODUCTION COMPLETE");
    println!("================================================================");
    println!("  Final z: {:.4}", 0.0);
    ...
    println!("  Snapshots: {} files", STEPS / SNAPSHOT_INTERVAL + 1);
...
fn save_snapshot(sim: &GpuNBodyTwoPass, path: &std::path::Path, step: usize, z: f64, box_size: f64) {
    ...
    // Header: n (u32), box (f32), step (u32), z (f32)
    ...
    // Particles: pos (3×f32), vel (3×f32), sign (i8) = 25 bytes each
    for i in 0..n {
...
fn compute_segregation_metrics(positions: &[f32], signs: &[i8], box_size: f64, n_cells: usize) -> (f64, f64, f64) {
    ...
    // Metric 1: Diff/Pois = Var(n+ - n-) / (n̄+ + n̄-)
    ...
    let corr = if var_plus > 0.0 && var_minus > 0.0 {
        cov / (var_plus.sqrt() * var_minus.sqrt())
    } else {
        0.0
    };

    // Metric 3: ExcVar- = (Var(n-) - n̄-) / n̄-
    let mut var_minus_count = 0.0;
    for i in 0..n_cells_total {
        var_minus_count += (n_minus[i] as f64 - mean_minus).powi(2);
    }
