//! JANUS V13 — Resume from snapshot 1400
...
fn main() {
    ...
    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║  JANUS V13 — RESUME from step {}                       ║", RESUME_STEP);
    println!("╚═══════════════════════════════════════════════════════════╝");
    ...
    // Calculate state at resume step
...
fn load_snap(path: &str) -> (Vec<f64>, Vec<f64>, Vec<i32>, usize) {
    ...
    for _ in 0..n {
        ...
        pos.push(f32::from_le_bytes(buf4) as f64);
        r.read_exact(&mut buf4).unwrap();
        pos.push(f32::from_le_bytes(buf4) as f64);
        r.read_exact(&mut buf4).unwrap();
        pos.push(f32::from_le_bytes(buf4) as f64);
        // vx, vy, vz
        r.read_exact(&mut buf4).unwrap();
        vel.push(f32::from_le_bytes(buf4) as f64);
        r.read_exact(&mut buf4).unwrap();
        vel.push(f32::from_le_bytes(buf4) as f64);
        r.read_exact(&mut buf4).unwrap();
        vel.push(f32::from_le_bytes(buf4) as f64);
        // sign
        r.read_exact(&mut buf4).unwrap();
        let s = f32::from_le_bytes(buf4);
        signs.push(if s > 0.0 { 1 } else { -1 });
    }
