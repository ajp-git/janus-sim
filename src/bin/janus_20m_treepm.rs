//! Janus 20M particle simulation with TreePM + Morton + warp-coherent
//!
...
//!   - Expected: ~13s/step @ 20M → 43h for 12000 steps
//!
//! Output: /app/output/20M_treepm_YYYY-MM-DD/
//!
//! N_max RTX 3060 12GB = 21M (FIX-013). Using 20M for safety margin.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║   Janus 20M TreePM Production Run (N_max RTX 3060)             ║");
    ...
    // Estimate runtime
    let estimated_step_ms = 13_400.0;  // ~13.4s/step measured on test_vram_limit 20M
    ...
    let output_base = format!("/app/output/20M_treepm_{}", date);
