...
// ════════════════════════════════════════════════════════════════
// PHASE 0 — SENTINELLES
// (μ, L, mass_factor_override, label)
// mass_factor_override < 0 means use calculated value: Ω_b × (1+μ) / 0.3
// ════════════════════════════════════════════════════════════════
const SENTINEL_CONFIGS: [(f64, f64, f64, &str); 5] = [
    (2.0,  100.0, -1.0,  "S1_mu2_L100"),           // calculated mf = 0.5
    (2.0,  50.0,  -1.0,  "S2_mu2_L50"),            // calculated mf = 0.5
    (19.0, 50.0,  -1.0,  "S3_mu19_L50"),           // calculated mf = 3.33
    (19.0, 100.0, -1.0,  "S4_mu19_L100"),          // calculated mf = 3.33
    (2.0,  100.0, 3.333, "S5_CONTROL_mu2_mf3.33"), // FORCED mf = 3.33 (same gravity as μ=19)
];
...
fn main() {
    ...
    // ─────────────────────────────────────────────
    println!("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  PHASE 0 — Sentinelles diagnostiques (5 runs)");
    println!("  S5 = CONTROL: μ=2 with FORCED mass_factor=3.33");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    let mut sentinel_results: Vec<RunResult> = Vec::new();

    for (i, (mu, l_box, mf_override, label)) in SENTINEL_CONFIGS.iter().enumerate() {
        total_runs_done += 1;
        println!("[Sentinel {}/5] μ={}, L={} ({})", i + 1, mu, l_box, label);

        let result = run_single_simulation(0, total_runs_done, label.to_string(), *mu, *l_box, *mf_override, &outdir);
        ...
        println!("  → ρ+_max/ρ+_mean = {:.1}, v_rms = {:.0} km/s, status = {} ({:.0}s)",
            collapse_ratio, result.metrics_zfinal.v_rms, result.status, result.wall_time_sec);

        sentinel_results.push(result);
    }

    // ─────────────────────────────────────────────
    // CRITÈRE GO/NO-GO — Check μ=19 (Janus model), not μ=2 (baseline)
    // S4 = index 3 = (μ=19, L=100)
    // ─────────────────────────────────────────────
    let s4 = &sentinel_results[3];  // μ=19, L=100
    let s4_ratio = if s4.rho_plus_mean > 0.0 {
        s4.metrics_zfinal.rho_plus_max / s4.rho_plus_mean
    } else { 0.0 };
    let s4_vrms = s4.metrics_zfinal.v_rms;

    println!("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  CRITÈRE GO/NO-GO (checking μ=19 Janus model)");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  S4 (μ=19, L=100): ρ+_max/ρ+_mean = {:.2} (seuil {})", s4_ratio, SENTINEL_RHO_RATIO_THRESHOLD);
    println!("  S4 (μ=19, L=100): v_rms = {:.0} km/s (seuil {})", s4_vrms, SENTINEL_VRMS_THRESHOLD);

    let go = s4_ratio >= SENTINEL_RHO_RATIO_THRESHOLD
          && s4_vrms >= SENTINEL_VRMS_THRESHOLD
          && s4.status == "OK";

    if !go {
        println!("\n❌ NO-GO: S4 (μ=19, L=100) ne collapse pas suffisamment.");
        println!("   Le scan complet ne sera PAS lancé.");

        let diag_path = format!("{}/SENTINEL_DIAGNOSIS.txt", outdir);
        let mut diag = File::create(&diag_path).unwrap();
        writeln!(diag, "SENTINEL DIAGNOSIS").unwrap();
        writeln!(diag, "==================").unwrap();
        writeln!(diag, "S4 (μ=19, L=100): ρ+_max/ρ+_mean = {:.2}, v_rms = {:.0}", s4_ratio, s4_vrms).unwrap();
        writeln!(diag, "Threshold: {} for ratio, {} for v_rms", SENTINEL_RHO_RATIO_THRESHOLD, SENTINEL_VRMS_THRESHOLD).unwrap();
        writeln!(diag, "Result: NO-GO, scan not launched").unwrap();

        let total_h = global_start.elapsed().as_secs_f64() / 3600.0;
        println!("\nTotal time: {:.2}h", total_h);
        std::process::exit(1);
    }
    ...
    // ─────────────────────────────────────────────
    println!("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  PHASE 1 — Scan principal {} μ × {} L = {} runs", SCAN_MU.len(), SCAN_L.len(), SCAN_MU.len() * SCAN_L.len());
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
