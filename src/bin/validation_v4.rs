//! Janus Validation Run — Protocol v4
//!
//! Validation runs comparing specific configurations:
//!   1. ε=0.15, k_min=3, H=0.01, η=1.045
//!   2. ε=0.25, k_min=3, H=0.01, η=1.045
//!
//! Usage:
//!   cargo run --release --features cuda,cufft --bin validation_v4
//!
//! Output:
...
const DT: f64 = 0.01;
const TOTAL_STEPS: usize = 5000;       // Full validation run
const SNAPSHOT_INTERVAL: usize = 100;  // Save snapshots
...
// Compare ε=0.15 vs ε=0.25, keeping other params fixed
const EPSILON_VALUES: &[f64] = &[0.15, 0.25];

// Fixed at k_min=3 (best for filaments from exploration)
const K_MIN_VALUES: &[usize] = &[3];

// Fixed at H=0.01
const HUBBLE_VALUES: &[f64] = &[0.01];

// Fixed at η=1.045
const ETA_VALUES: &[f64] = &[1.045];
...
fn main() {
    println!("═══════════════════════════════════════════════════════════════");
    println!("  JANUS VALIDATION RUNS — Protocol v4");
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    ...
    let csv_path = format!("{}/validation_results.csv", results_dir);
