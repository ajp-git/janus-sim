//! Test 1M particles with multi-mode Zel'dovich ICs
//! Verify no sheet artifacts in rendered frames
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let output_dir = "/app/output/1M_multimode_test";
    let snapshot_dir = format!("{}/snapshots", output_dir);
    fs::create_dir_all(&snapshot_dir)?;

    eprintln!("1M MULTI-MODE ZEL'DOVICH TEST");
    eprintln!("Verify no sheet artifacts in rendered frames");
    eprintln!();
    ...
    let dtau_per_step = (tau_z_final - tau_z_init) / 6000.0;  // Match 40M timing
    ...
    eprintln!();
    eprintln!("Done! Check frames for sheet artifacts.");
