//! Janus 40M particle simulation with TreePM + Morton + warp-coherent
//!
...
//!
//! Output: /app/output/40M_treepm_YYYY-MM-DD/
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║   Janus 40M TreePM Production Run                              ║");
    ...
    // Estimate runtime
    let estimated_step_ms = 15_000.0;  // ~15s/step based on extrapolation from 85M
    ...
    let output_base = format!("/app/output/40M_treepm_{}", date);
